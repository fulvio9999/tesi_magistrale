...
import os, time, sys
from dataset import MIL_Dataset
...
parser.add_argument('--train_iters', type=int, default=5000, metavar='N',
                    help='number of train iters to train (default: 5000)')
...
class BSN(object):
    def __init__(self, dataset):
        ...
        self.snapshot = 50
        self.model_save_dir = 'BSN_tensorflow/' + args.model_save_dir
        self.batch_size = 1
...
if __name__ == '__main__':
    seed = [args.seed+i*5 for i in range(5)]
    acc_minet = np.zeros((args.run, args.folds))
    acc = np.zeros((args.run, args.folds))
    for idx in range(args.run):
        dataset = MIL_Dataset(seed[idx], path, args.folds)
        for fold in range(args.folds):
            print('run=', idx, ' fold=', fold)
            net = BSN(dataset)
            # _, tr_data = net.run_net1(fold)
            acc_minet[idx][fold], tr_data = net.run_net1(fold)
            tf.reset_default_graph()
            
            acc[idx][fold], _ = net.run_net2(tr_data, fold)
            tf.reset_default_graph()
        print(f'acc run {idx}:')
        print('MI-Net: mean=', np.mean(acc_minet[idx]), ' std=', np.std(acc_minet[idx]))
        print('BSN   : mean=', np.mean(acc[idx]), ' std=', np.std(acc[idx]))

    print(acc)
    print('acc FINALE:')
    print('MI-Net: mean=', np.mean(acc_minet), ' std=', np.std(acc_minet))
    print('BSN   : mean=', np.mean(acc), ' std=', np.std(acc))
