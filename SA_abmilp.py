...
class SA_ABMILP(nn.Module):
    def __init__(self, self_att, input_dim):
    ...
    def calculate_classification_error(self, X, Y):
        ...
        Y_prob, Y_hat = Y_prob.squeeze(), Y_hat.squeeze()
    ...
    def calculate_objective(self, X, Y):
        ...
        Y_prob = Y_prob
        log_likelihood = -self.log_Bernoulli(Y, Y_prob)

        return log_likelihood.squeeze()#, A
    
    def log_Bernoulli(self, x, mean, average=False, dim=1):
        min_epsilon = 1e-5
        max_epsilon = 1.-1e-5
